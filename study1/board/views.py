...
def list(request) :
    ...
    # Paginator : all_boards 목록을 10개씩 묶어서 분리 저장.
    paginator = Paginator(all_boards,10)
    # pagenator 객체에서 pageNum번째 게시물 리턴
...
def info(request,num) :
    board = Board.objects.get(num=num) # num 값에 해당하는 게시물 한 개 저장 
    board.readcnt += 1  # 조회수값 1 증가
...
#    2. 내용 수정
#       수정 완료 : 상세보기 페이지 이동
#       수정 실패 : 메세지 출력 후, update 화면 페이지 이동
        board = Board.objects.get(num=num)
        pass1 = request.POST["pass"]
        ...
        try :
            ...
            b = Board(num=num,name=request.POST["name"],
                      pass1 = request.POST["pass"],
                      subject = request.POST["subject"],
                      content = request.POST["content"],
                      file1 = filename)
            ...
            print(traceback.format_exc()) #runserver 콘솔창에 오류 메세지
            context = {"msg":"게시물 수정 오류",
                       "url":"../../update/"+str(num) + "/"}
            return render(request,"alert.html",context)
...
#    1. 비밀전호 검증.
#       오류 : 비밀번호 오류 메세지 출력. delete 화면 페이지 이동
#    2. 해당 게시물 삭제.
#       게시물 목록(list) 페이지 출력
